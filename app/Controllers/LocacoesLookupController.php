...
final class LocacoesLookupController extends Controller
{
    private const PER_PAGE = 20;

    /** @var array<string, string> */
    private const TYPES = [
        'locacoes_obs_default' => 'Observações padrão (entregador)',
        'locacoes_regiao' => 'Regiões / rotas',
    ];

    private const BASE = '/locacoes/opcoes-auxiliares';

    public function index(Request $request): string
    {
        $cid = $this->requireCompany();
        $type = (string) $request->input('type', 'locacoes_obs_default');
        if (!isset(self::TYPES[$type])) {
            $type = 'locacoes_obs_default';
        }
        ...
        return $this->view('locacoes/lookups/index', [
            'title' => 'Opções auxiliares — Locações',
    ...
    public function create(Request $request): string
    {
        ...
        return $this->view('locacoes/lookups/form', [
            'title' => 'Novo item',
            'pageTitle' => 'Novo — ' . self::TYPES[$type],
            'breadcrumbs' => [
                ['label' => 'Início', 'href' => '/dashboard'],
                ['label' => 'Locações', 'href' => self::BASE . '?type=' . rawurlencode($type)],
