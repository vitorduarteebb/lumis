...
final class EstoqueLookupController extends Controller
{
    private const PER_PAGE = 20;

    /** @var array<string, string> */
    private const TYPES = [
        'estoque_adjust_reason' => 'Motivos de ajuste',
        'estoque_movement_label' => 'Tipos / rótulos de movimentação',
        'estoque_purchase_status' => 'Status de compra',
        'estoque_quote_status' => 'Status de cotação',
        'estoque_return_reason' => 'Motivos de troca e devolução',
    ];

    private const BASE = '/estoque/opcoes-auxiliares';

    public function index(Request $request): string
    {
        $cid = $this->requireCompany();
        $type = (string) $request->input('type', 'estoque_adjust_reason');
        if (!isset(self::TYPES[$type])) {
            $type = 'estoque_adjust_reason';
        }
        ...
        return $this->view('estoque/lookups/index', [
    ...
    public function create(Request $request): string
    {
        ...
        return $this->view('estoque/lookups/form', [
            'title' => 'Novo item',
    ...
    public function store(Request $request): string
    {
        ...
        $repo->insert($cid, ['entry_type' => $type, 'name' => $name, 'slug' => $slug, 'sort_order' => $sort, 'status' => $status]);
    ...
    public function update(Request $request): string
    {
        ...
        $repo->update($id, $cid, ['name' => $name, 'slug' => $slug, 'sort_order' => $sort, 'status' => $status]);
