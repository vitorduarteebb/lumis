...
final class SupplierRepository extends BaseRepository
{
    ...
    {
        $sql = 'SELECT * FROM suppliers WHERE id = :id AND company_id = :cid AND deleted_at IS NULL LIMIT 1';
    ...
    public function paginate(int $companyId, string $search, ?string $statusFilter, int $page, int $perPage): array
    {
        ...
        $sql = "SELECT * FROM suppliers WHERE {$whereSql} ORDER BY name ASC LIMIT " . (int) $perPage . ' OFFSET ' . (int) $offset;
        $stmt = $this->pdo()->prepare($sql);
        $stmt->execute($params);
    ...
    public function softDelete(int $id, int $companyId): void
    {
        $stmt = $this->pdo()->prepare(
            'UPDATE suppliers SET deleted_at = NOW(), updated_at = NOW() WHERE id = :id AND company_id = :cid AND deleted_at IS NULL'
        );
        $stmt->execute(['id' => $id, 'cid' => $companyId]);
    }
}
