...
final class ContractServiceRepository extends BaseRepository
{
    /**
    ...
    public function paginate(
        ...
        if ($dateFrom !== null && $dateFrom !== '') {
            $where[] = 'COALESCE(t.start_date, t.created_at) >= :df';
            $params['df'] = $dateFrom;
        }
        ...
        if ($search !== '') {
            $where[] = '(t.contract_number LIKE :sq OR t.description LIKE :sq OR c.name LIKE :sq OR sv.name LIKE :sq)';
            $params['sq'] = '%' . $search . '%';
        }
        $whereSql = implode(' AND ', $where);
        $stmt = $this->pdo()->prepare(
            "SELECT COUNT(*) FROM contract_services t
        ...
        $sql = "SELECT t.*, c.name AS client_name, sv.name AS service_name
                FROM contract_services t
                LEFT JOIN clients c ON c.id = t.client_id
                LEFT JOIN services sv ON sv.id = t.service_id
                WHERE {$whereSql}
                ORDER BY t.id DESC LIMIT " . (int) $perPage . ' OFFSET ' . (int) $offset;
    ...
    {
        $stmt = $this->pdo()->prepare(
            'SELECT t.*, c.name AS client_name, sv.name AS service_name
             FROM contract_services t
             LEFT JOIN clients c ON c.id = t.client_id
             LEFT JOIN services sv ON sv.id = t.service_id
    ...
    public function insert(int $companyId, array $data, ?int $userId): int
    {
        $stmt = $this->pdo()->prepare(
            'INSERT INTO contract_services (
                company_id, store_id, contract_number, client_id, service_id, description, start_date, end_date, amount, adjustment_note,
                periodicity_entry_id, status, notes, attachment_path, created_by, created_at, updated_at
            ) VALUES (
                :cid, :sid, :cnum, :clid, :sv, :desc, :sd, :ed, :amt, :adj,
        ...
        $stmt->execute([
            ...
            'clid' => $data['client_id'],
            'sv' => $data['service_id'] ?? null,
            'desc' => $data['description'] ?? null,
            'sd' => $data['start_date'] ?? null,
            'ed' => $data['end_date'] ?? null,
            'amt' => $data['amount'] ?? 0,
            'adj' => $data['adjustment_note'] ?? null,
        ...
        if (empty($data['contract_number'])) {
            $this->pdo()->prepare('UPDATE contract_services SET contract_number = :n WHERE id = :id')->execute([
                'n' => 'CS-' . $id,
                'id' => $id,
            ]);
        }
    ...
    {
        $stmt = $this->pdo()->prepare(
            'UPDATE contract_services SET
                store_id = :sid, contract_number = :cnum, client_id = :clid, service_id = :sv, description = :desc,
                start_date = :sd, end_date = :ed, amount = :amt, adjustment_note = :adj, periodicity_entry_id = :per,
    ...
    {
        $this->pdo()->prepare(
            'UPDATE contract_services SET status = :st, updated_by = :ub, updated_at = NOW() WHERE id = :id AND company_id = :cid AND deleted_at IS NULL'
    ...
    {
        $this->pdo()->prepare(
            'UPDATE contract_services SET attachment_path = :p, updated_by = :ub, updated_at = NOW() WHERE id = :id AND company_id = :cid AND deleted_at IS NULL'
