...
$basePath = (string) ($basePath ?? '/financeiro/contas-receber');
...
<div class="lumis-page-head">
    <div>
        ...
        <h2 class="h4 mb-1 text-white">Contas a receber</h2>
        <div class="text-secondary small">Cobranças de clientes com baixa parcial.</div>
    </div>
</div>
...
<?php if ($editRow !== null): ?>
    ...
    $ecid = $editRow['client_id'] !== null ? (int) $editRow['client_id'] : 0;
    ?>
    <div class="card app-card border-secondary-subtle mb-4">
        <div class="card-body">
            <h3 class="h6 text-white mb-3">Editar título #<?= $eid ?></h3>
            <form method="post" action="/financeiro/contas-receber" class="row g-3 align-items-end">
                ...
                <div class="col-md-3">
                    <label class="form-label lumis-label" for="ed_cli">Cliente</label>
                    <select class="form-select app-input" id="ed_cli" name="client_id">
                        <option value="">—</option>
                        <?php foreach ($clients as $c): ?>
                            <option value="<?= (int) ($c['id'] ?? 0) ?>" <?= $ecid === (int) ($c['id'] ?? 0) ? 'selected' : '' ?>>
        ...
        <div class="card-body">
            ...
            <form method="post" action="/financeiro/contas-receber" class="row g-3 align-items-end">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control app-input" id="nw_desc" name="description" required placeholder="Ex.: Pedido / fatura">
                </div>
                <div class="col-md-3">
                    <label class="form-label lumis-label" for="nw_cli">Cliente</label>
                    <select class="form-select app-input" id="nw_cli" name="client_id">
                        <option value="">—</option>
                        <?php foreach ($clients as $c): ?>
...
<form method="get" action="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>" class="lumis-toolbar mb-3 d-flex flex-wrap gap-2 align-items-end">
    <select name="status" class="form-select app-input" style="max-width: 200px;">
        ...
        <option value="paid" <?= $statusFilter === 'paid' ? 'selected' : '' ?>>Recebido</option>
        ...
        <tbody>
            ...
            <?php foreach ($rows as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-secondary small"><?= $cn !== '' ? htmlspecialchars($cn, ENT_QUOTES, 'UTF-8') : '—' ?></td>
                    ...
                    <td class="text-end">
                        <?php if (can('financeiro.contas_receber.view')): ?>
                            <a class="btn btn-sm btn-lumis-secondary" href="<?= htmlspecialchars($basePath . '?edit=' . $rid, ENT_QUOTES, 'UTF-8') ?>">Editar</a>
                            <?php if ($st === 'open'): ?>
                                <form method="post" action="/financeiro/contas-receber" class="d-inline-flex gap-1 align-items-center">
                                    <?= \App\Helpers\Csrf::field() ?>
                                    <input type="hidden" name="_action" value="receive">
                                    <input type="hidden" name="id" value="<?= $rid ?>">
                                    <input type="text" name="receipt_amount" class="form-control form-control-sm app-input" style="width: 88px;" placeholder="Valor" title="Valor a receber">
                                    <button type="submit" class="btn btn-sm btn-primary">Receber</button>
                                </form>
                            <?php endif; ?>
                            <form method="post" action="/financeiro/contas-receber" class="d-inline" onsubmit="return confirm('Excluir este título?');">
                                <?= \App\Helpers\Csrf::field() ?>
