...
$currentType = (string) ($currentType ?? 'os_status');
$rows = $rows ?? [];
$basePath = (string) ($basePath ?? '/ordens-servico/opcoes-auxiliares');
...
<div class="lumis-page-head">
    <div>
        ...
        <div class="text-secondary small">Status, prioridades, tipos e categorias.</div>
    </div>
    <?php if (can('ordens_servico.opcoes_auxiliares.create')): ?>
        <a class="btn btn-primary btn-sm rounded-3" href="/ordens-servico/opcoes-auxiliares/novo?type=<?= htmlspecialchars(rawurlencode($currentType), ENT_QUOTES, 'UTF-8') ?>">Novo item</a>
    <?php endif; ?>
</div>
        ...
        <tbody>
            ...
            <?php foreach ($rows as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <?php if (can('ordens_servico.opcoes_auxiliares.edit')): ?>
                            <a class="btn btn-sm btn-lumis-secondary" href="/ordens-servico/opcoes-auxiliares/<?= $rid ?>/editar">Editar</a>
                        <?php endif; ?>
                        <?php if (can('ordens_servico.opcoes_auxiliares.delete')): ?>
                            <form method="post" action="/ordens-servico/opcoes-auxiliares/<?= $rid ?>/excluir" class="d-inline" onsubmit="return confirm('Excluir este item?');">
