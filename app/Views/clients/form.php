...
$mode = $mode ?? 'create';
$client = is_array($client ?? null) ? $client : null;
$errors = is_array($errors ?? null) ? $errors : [];
$old = is_array($old ?? null) ? $old : [];
$isEdit = $mode === 'edit';
$action = $isEdit ? '/cadastros/clientes/' . (int) ($client['id'] ?? 0) : '/cadastros/clientes';

$val = static function (string $key, string $default = '') use ($old, $client, $isEdit): string {
    if (array_key_exists($key, $old)) {
        return (string) $old[$key];
    }
    if ($isEdit && $client !== null && array_key_exists($key, $client) && $client[$key] !== null) {
        return (string) $client[$key];
    }

    return $default;
};
$pt = $val('person_type', $isEdit ? 'F' : 'F');
...
<div class="lumis-page-head">
    <div>
        ...
        <h2 class="h4 mb-1 text-white"><?= $isEdit ? 'Editar cliente' : 'Novo cliente' ?></h2>
    </div>
    <a class="btn btn-lumis-secondary btn-sm rounded-3" href="/cadastros/clientes">Voltar</a>
</div>
...
<form class="lumis-form" method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="d-flex flex-wrap gap-2 justify-content-end pt-2">
        <a class="btn btn-lumis-secondary" href="/cadastros/clientes">Cancelar</a>
