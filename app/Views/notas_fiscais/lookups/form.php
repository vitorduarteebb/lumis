...
$entryType = (string) ($entryType ?? 'fiscal_series');
$typeLabel = (string) ($typeLabel ?? '');
$errors = is_array($errors ?? null) ? $errors : [];
$old = is_array($old ?? null) ? $old : [];
$isEdit = $mode === 'edit';
$base = '/notas-fiscais/opcoes-auxiliares';
...
<form class="lumis-form" method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="lumis-form-section row g-3">
        ...
        <div class="col-md-4">
            ...
            <input type="text" class="form-control app-input <?= isset($errors['slug']) ? 'is-invalid' : '' ?>" name="slug" value="<?= htmlspecialchars($val('slug'), ENT_QUOTES, 'UTF-8') ?>">
        </div>
        <div class="col-12">
            <label class="form-label lumis-label">Código / observação auxiliar (CFOP, texto administrativo…)</label>
            <textarea class="form-control app-input" name="value_text" rows="3"><?= htmlspecialchars($val('value_text'), ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>
        <div class="col-md-4">
