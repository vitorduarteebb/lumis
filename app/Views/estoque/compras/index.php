...
$total = (int) ($total ?? 0);
$basePath = (string) ($basePath ?? '/estoque/compras');
...
<div class="lumis-page-head">
    <div>
        ...
        <h2 class="h4 mb-1 text-white">Compras</h2>
        <div class="text-secondary small">Pedidos de compra com entrada de estoque ao finalizar.</div>
    </div>
    <?php if (can('estoque.compras.create')): ?>
        <a class="btn btn-primary btn-sm rounded-3" href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/novo">Nova compra</a>
    <?php endif; ?>
</div>
...
<div class="lumis-table-wrap mb-2">
    <table class="table lumis-table mb-0">
        ...
        <tbody>
            <?php if ($rows === []): ?>
                <tr><td colspan="6" class="text-secondary small py-4">Nenhuma compra.</td></tr>
            <?php endif; ?>
            <?php foreach ($rows as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-secondary small"><?= htmlspecialchars((string) ($row['supplier_name'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td>
                    <td>
                        <?php if ($st === 'finalized'): ?>
