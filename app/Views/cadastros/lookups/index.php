...
$currentType = (string) ($currentType ?? 'client_type');
$rows = $rows ?? [];
$basePath = (string) ($basePath ?? '/cadastros/opcoes-auxiliares');
...
<div class="lumis-page-head">
    <div>
        ...
        <div class="text-secondary small">Tabelas de apoio para cadastros.</div>
    </div>
    <?php if (can('cadastros.opcoes_auxiliares.create')): ?>
        <a class="btn btn-primary btn-sm rounded-3" href="/cadastros/opcoes-auxiliares/novo?type=<?= htmlspecialchars(rawurlencode($currentType), ENT_QUOTES, 'UTF-8') ?>">Novo item</a>
    <?php endif; ?>
</div>
        ...
        <tbody>
            ...
            <?php foreach ($rows as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <?php if (can('cadastros.opcoes_auxiliares.edit')): ?>
                            <a class="btn btn-sm btn-lumis-secondary" href="/cadastros/opcoes-auxiliares/<?= $rid ?>/editar">Editar</a>
                        <?php endif; ?>
                        <?php if (can('cadastros.opcoes_auxiliares.delete')): ?>
                            <form method="post" action="/cadastros/opcoes-auxiliares/<?= $rid ?>/excluir" class="d-inline" onsubmit="return confirm('Excluir este item?');">
