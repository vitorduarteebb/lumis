...
$entryType = (string) ($entryType ?? 'client_type');
$typeLabel = (string) ($typeLabel ?? '');
$errors = is_array($errors ?? null) ? $errors : [];
$old = is_array($old ?? null) ? $old : [];
$isEdit = $mode === 'edit';
$action = $isEdit ? '/cadastros/opcoes-auxiliares/' . (int) ($row['id'] ?? 0) : '/cadastros/opcoes-auxiliares';
...
<div class="lumis-page-head">
    ...
    <a class="btn btn-lumis-secondary btn-sm rounded-3" href="/cadastros/opcoes-auxiliares?type=<?= htmlspecialchars(rawurlencode($entryType), ENT_QUOTES, 'UTF-8') ?>">Voltar</a>
</div>
...
<form class="lumis-form" method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="d-flex flex-wrap gap-2 justify-content-end pt-2">
        <a class="btn btn-lumis-secondary" href="/cadastros/opcoes-auxiliares?type=<?= htmlspecialchars(rawurlencode($entryType), ENT_QUOTES, 'UTF-8') ?>">Cancelar</a>
