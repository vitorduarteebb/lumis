...
$basePath = (string) ($basePath ?? '/contratos/assinaturas');
...
<div class="lumis-page-head">
    <div>
        ...
        <h2 class="h4 mb-1 text-white">Assinaturas</h2>
        <div class="text-secondary small">Recorrência e próximas cobranças.</div>
    </div>
    <?php if (can('contratos.assinaturas.create')): ?>
        <a class="btn btn-primary btn-sm rounded-3" href="/contratos/assinaturas/novo">Nova assinatura</a>
    <?php endif; ?>
</div>
<form method="get" action="/contratos/assinaturas" class="lumis-toolbar mb-3 d-flex flex-wrap gap-2 align-items-end">
    <div class="input-group lumis-search flex-grow-1" style="max-width: 280px;">
        ...
        <input type="search" name="q" class="form-control app-input" placeholder="Número, plano, cliente…" value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>">
    </div>
...
<div class="lumis-table-wrap mb-2">
    <table class="table lumis-table mb-0">
        ...
        <tbody>
            <?php if ($rows === []): ?>
                <tr><td colspan="7" class="text-secondary small py-4">Nenhuma assinatura.</td></tr>
            <?php endif; ?>
            <?php foreach ($rows as $row): ?>
                ...
                ?>
                <tr>
                    <td class="text-white font-monospace small"><?= htmlspecialchars((string) ($row['subscription_number'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td>
                    <td class="text-secondary small"><?= htmlspecialchars((string) ($row['client_name'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td>
                    <td class="text-secondary small"><?= htmlspecialchars(mb_substr((string) ($row['plan_description'] ?? '—'), 0, 40), ENT_QUOTES, 'UTF-8') ?></td>
                    ...
                    <td class="text-end text-white small"><?= htmlspecialchars(lumis_money_br((float) ($row['recurring_amount'] ?? 0)), ENT_QUOTES, 'UTF-8') ?></td>
                    <td class="text-secondary small"><?= htmlspecialchars(substr((string) ($row['next_billing_date'] ?? ''), 0, 10), ENT_QUOTES, 'UTF-8') ?></td>
                    <td class="text-end"><a class="btn btn-sm btn-lumis-secondary" href="/contratos/assinaturas/<?= $rid ?>">Ver</a></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
