...
$basePath = (string) ($basePath ?? '/contratos/locacoes');
...
<div class="lumis-page-head">
    <div>
        ...
        <h2 class="h4 mb-1 text-white">Locações</h2>
        <div class="text-secondary small">Contratos de locação de bens.</div>
    </div>
    <?php if (can('contratos.locacoes.create')): ?>
        <a class="btn btn-primary btn-sm rounded-3" href="/contratos/locacoes/novo">Novo contrato</a>
    <?php endif; ?>
</div>
<form method="get" action="/contratos/locacoes" class="lumis-toolbar mb-3 d-flex flex-wrap gap-2 align-items-end">
    <div class="input-group lumis-search flex-grow-1" style="max-width: 280px;">
        ...
        <input type="search" name="q" class="form-control app-input" placeholder="Número, bem, cliente…" value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>">
    </div>
...
<div class="lumis-table-wrap mb-2">
    <table class="table lumis-table mb-0">
        ...
        <tbody>
            <?php if ($rows === []): ?>
                <tr><td colspan="6" class="text-secondary small py-4">Nenhum contrato.</td></tr>
            <?php endif; ?>
            <?php foreach ($rows as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-secondary small"><?= htmlspecialchars(mb_substr((string) ($row['asset_description'] ?? '—'), 0, 60), ENT_QUOTES, 'UTF-8') ?></td>
                    ...
                    <td class="text-end"><a class="btn btn-sm btn-lumis-secondary" href="/contratos/locacoes/<?= $rid ?>">Ver</a></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
