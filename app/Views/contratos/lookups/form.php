...
$entryType = (string) ($entryType ?? 'contract_type');
$typeLabel = (string) ($typeLabel ?? '');
$errors = is_array($errors ?? null) ? $errors : [];
$old = is_array($old ?? null) ? $old : [];
$isEdit = $mode === 'edit';
$base = '/contratos/opcoes-auxiliares';
...
<form class="lumis-form" method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="lumis-form-section row g-3">
        ...
        <div class="col-12">
            <label class="form-label lumis-label">Texto auxiliar / modelo (opcional)</label>
            <textarea class="form-control app-input" name="value_text" rows="4"><?= htmlspecialchars($val('value_text'), ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>
        <div class="col-md-4">
