...
$currentType = (string) ($currentType ?? 'quote_status');
$rows = $rows ?? [];
$basePath = (string) ($basePath ?? '/orcamentos/opcoes-auxiliares');
...
<div class="lumis-page-head">
    <div>
        ...
        <div class="text-secondary small">Status, condições de pagamento e modelos de observações.</div>
    </div>
    <?php if (can('orcamentos.opcoes_auxiliares.create')): ?>
        <a class="btn btn-primary btn-sm rounded-3" href="/orcamentos/opcoes-auxiliares/novo?type=<?= htmlspecialchars(rawurlencode($currentType), ENT_QUOTES, 'UTF-8') ?>">Novo item</a>
    <?php endif; ?>
</div>
        ...
        <tbody>
            ...
            <?php foreach ($rows as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <?php if (can('orcamentos.opcoes_auxiliares.edit')): ?>
                            <a class="btn btn-sm btn-lumis-secondary" href="/orcamentos/opcoes-auxiliares/<?= $rid ?>/editar">Editar</a>
                        <?php endif; ?>
                        <?php if (can('orcamentos.opcoes_auxiliares.delete')): ?>
                            <form method="post" action="/orcamentos/opcoes-auxiliares/<?= $rid ?>/excluir" class="d-inline" onsubmit="return confirm('Excluir este item?');">
