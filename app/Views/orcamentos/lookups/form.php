...
$entryType = (string) ($entryType ?? 'quote_status');
$typeLabel = (string) ($typeLabel ?? '');
$errors = is_array($errors ?? null) ? $errors : [];
$old = is_array($old ?? null) ? $old : [];
$isEdit = $mode === 'edit';
$action = $isEdit ? '/orcamentos/opcoes-auxiliares/' . (int) ($row['id'] ?? 0) : '/orcamentos/opcoes-auxiliares';
...
$showVt = $entryType === 'quote_note_template';
?>
<div class="lumis-page-head">
    ...
    <a class="btn btn-lumis-secondary btn-sm rounded-3" href="/orcamentos/opcoes-auxiliares?type=<?= htmlspecialchars(rawurlencode($entryType), ENT_QUOTES, 'UTF-8') ?>">Voltar</a>
</div>
...
<form class="lumis-form" method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="lumis-form-section">
        <div class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <input type="text" class="form-control app-input <?= isset($errors['slug']) ? 'is-invalid' : '' ?>" id="slug" name="slug" placeholder="gerado automaticamente se vazio" value="<?= htmlspecialchars($val('slug'), ENT_QUOTES, 'UTF-8') ?>">
                <?php if (isset($errors['slug'])): ?><div class="invalid-feedback"><?= htmlspecialchars($errors['slug'], ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?>
            </div>
            <?php if ($showVt): ?>
                <div class="col-12">
                    <label class="form-label lumis-label" for="value_text">Texto do modelo</label>
                    <textarea class="form-control app-input" id="value_text" name="value_text" rows="4"><?= htmlspecialchars($val('value_text'), ENT_QUOTES, 'UTF-8') ?></textarea>
                </div>
            <?php endif; ?>
            <div class="col-md-4">
    ...
    <div class="d-flex flex-wrap gap-2 justify-content-end pt-2">
        <a class="btn btn-lumis-secondary" href="/orcamentos/opcoes-auxiliares?type=<?= htmlspecialchars(rawurlencode($entryType), ENT_QUOTES, 'UTF-8') ?>">Cancelar</a>
